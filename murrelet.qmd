...
```{r landscape, warning=FALSE, message=FALSE, echo=TRUE}
#| code-fold: true
#| code-summary: "Show/Hide Code"
...
![Core Areas from the Howe Sound Classification based on the NLCD Cover Types](images/murrelet_LandCover_cores.png)
...
Below is an example the Old Growth Management Areas (OGMAs) and the mapped expected habitat of the Marbled Murrelet, and what the Grains of Connectivity analysis for the Marbled Murrelet might be for Gambier Island. Note that these birds can fly long distances and so the example is just to show what the GoC might look like for this landscape.

![Mapped OGMAs and Murrelet Habitat](images/murrelet_OGMA_habitat.png)

![Expected Grains of Connectivity on Gambier Island for Marbled Murrelet](images/murrelet_GoC.png)
